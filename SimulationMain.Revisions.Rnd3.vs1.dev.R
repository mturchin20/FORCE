...
#20210125 NOTE -- brought this code in below in case I wanted to do a duplciated SNP removal step, but for now not going to implement it. Current logic is that in data analysis you don't remove SNPs that are no where near one another if my chance they happen to have the same genotypes (unless a method explicitly requires it, but this is not typical).
