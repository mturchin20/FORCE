...
// [[Rcpp::export]]
List InterPath(mat X,vec y,mat GSM,mat G,mat M,List regions,int n,int nsnp,int cores = 1){
    int i;
...
//        mat b = zeros(n,j.n_elem+1); //NOTE -- original InterPath fcode
//        b.col(0) = ones<vec>(n); b.cols(1,j.n_elem) = trans(X.rows(j-1));
//        mat b = zeros(n,q+1); //NOTE -- my first set of edits to incorporate the Z matrix
//        b.col(0) = ones<vec>(n); b.cols(1,q) = Z.t();        
//	mat btb_inv = inv(b.t()*b); //NOTE -- this and the 3 following lines are now being commented out
...
//        eig_sym(eigval,eigvec,delta_null(0)*Kc+delta_null(1)*(eye<mat>(n,n)-(b*btb_inv)*b.t()));
        eig_sym(eigval,eigvec,delta_null(0)*Kc+delta_null(1)*M);
...
//        eig_sym(evals, (eigvec.cols(find(eigval>0))*diagmat(sqrt(eigval(find(eigval>0))))*trans(eigvec.cols(find(eigval>0))))*(Sinv(1,0)*Kc+Sinv(1,1)*Qc+Sinv(1,2)*(eye<mat>(n,n)-(b*btb_inv)*b.t()))*(eigvec.cols(find(eigval>0))*diagmat(sqrt(eigval(find(eigval>0))))*trans(eigvec.cols(find(eigval>0)))));
        eig_sym(evals, (eigvec.cols(find(eigval>0))*diagmat(sqrt(eigval(find(eigval>0))))*trans(eigvec.cols(find(eigval>0))))*(Sinv(1,0)*Kc+Sinv(1,1)*Qc+Sinv(1,2)*M)*(eigvec.cols(find(eigval>0))*diagmat(sqrt(eigval(find(eigval>0))))*trans(eigvec.cols(find(eigval>0)))));
        Lambda.col(i) = evals;
