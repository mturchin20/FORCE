...
// [[Rcpp::export]]
List MAPITR(vec y,mat X,mat GSM,mat regions,mat Z,int cores = 1){
    int i;
...
//        mat b = zeros(n,j.n_elem+1);
        mat b = zeros(n,j.n_elem+q+1);
	b.col(0) = ones<vec>(n); b.cols(1,q) = Z.t(); b.cols(q+1,q+1+j.n_elem) = trans(X.rows(j));
