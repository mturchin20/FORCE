...
// [[Rcpp::export]]
List InterPath(mat X,vec y,mat GSM,mat G,List regions,int n,int nsnp,int cores = 1){ 
    int i;
...
//        b.col(0) = ones<vec>(n); b.cols(1,j.n_elem) = X.rows(0,j.n_elem-1).t();
        b.col(0) = ones<vec>(n); b.cols(1,j.n_elem) = X.t();
