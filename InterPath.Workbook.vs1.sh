	...
	for (i in DataTypes[3]) { \
		...
		plotPoints1 <- list(); ListCounter1 <- 1; for (j in UKBioBankPops) { ancestry1 = strsplit(j, \";\")[[1]][1]; ancestry2 = strsplit(j, \";\")[[1]][2]; \	
			for (k in c(\"Height\", \"BMI\", \"WaistAdjBMI\", \"HipAdjBMI\")) { \
				...
				print(Paths1.Overlap); print(warnings()); \
				ggplot1 <- ggplot(Paths1.Overlap.Melted, aes(ind.1, ind.2, fill=value)) + geom_tile(colour=\"white\") + scale_fill_gradient(low = \"steelblue\", high=\"darkred\", na.value=\"white\") + geom_text(aes(ind.1, ind.2, label = value), color = \"black\", size = 6) + theme_minimal() + theme(axis.title.x = element_blank(), axis.title.y = element_blank(), panel.grid.major = element_blank(), panel.border = element_blank(), panel.background = element_blank(), axis.ticks = element_blank(), axis.text.x = element_text(angle = 90, hjust = 1)); print(warnings()); plotPoints1[[paste(\"P\", as.character(ListCounter1), sep=\"\")]] <- ggplot1; ListCounter1 <- ListCounter1 + 1; print(warnings()); \
			}; \
		}; print(length(plotPoints1)); grid.newpage(); pushViewport(viewport(layout = grid.layout(8, 5, height=unit(c(0.5,5,5,5,5,5,5,5), width=unit(.5,5,5,5,5), \"null\")))); \ 
		grid.text(\"Height\", vp=viewport(layout.pos.row=1, layout.pos.col=2)); grid.text(\"BMI\", vp=viewport(layout.pos.row=1, layout.pos.col=3)); grid.text(\"WaistAdjBMI\", vp=viewport(layout.pos.row=1, layout.pos.col=4)); grid.text(\"HipAdjBMI\", vp=viewport(layout.pos.row=1, layout.pos.col=5)); \
for (i2 in 1:length(UKBioBankPops)) { ancestry2 = strsplit(UKBioBankPops[i2], \";\")[[1]][2]; grid.text(ancestry2. vp=viewport(layout.pos.row=i2+1, layout.pos.col=1)); for (j2 in 1:4) { print(plotPoints1[1],  
	}; \
"
...
#scp -p
/Users/mturchin20/Documents/Work/LabMisc/RamachandranLab/InterPath/Vs1/Analyses/Rnd2AdditiveMdls/GK/ArchitectureExplore/.
...
module load R/3.4.3_mkl gcc; sleep 10800; for i in `cat <(echo "Height;1254 BMI;58923 Waist;49281 Hip;37485 WaistAdjBMI;82374 HipAdjBMI;6182" | perl -lane 'print join("\n", @F);') | grep -vE 'Waist;49|Hip;37' | tail -n 4 | tail -n 1`; do
...
#			NumPaths=2
			...
			duration=$SECONDS; echo "$(($duration / 60)) minutes and $(($duration % 60)) seconds elapsed."
						...
						VarDrop <- c(NA); 
						VarCount <- length(strsplit(strsplit(Pathways.wGenes[$PathNum,3], \";\")[[1]][i], \",\")[[1]]); if (neg.is.na(InterPath.output.Est[i,1]) && VarCount > 1) { \ 
						else if (VarCount == 1) { VarDrop <- c(VarDrop, as.character(Genes[i])); } \  
					Chisq.Stat <- -2 * sum(log(pVals1), na.rm=TRUE); Chisq.pVal <- \"PH\"; if (Chisq.Stat == 0) { Chisq.pVal <- -9; } else { Chisq.pVal <- pchisq(Chisq.Stat, df=2*length(Results1), lower.tail=FALSE); }; Results2 <- c(as.character(Pathways.wGenes[$PathNum,1]), Chisq.pVal, paste(Results1, collapse=\";\"), WeirdFlag1, length(Genes), length(Results1), paste(VarDrop, collapse=\",\")); \ 
